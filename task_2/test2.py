...
'''
    def finish(self):
        ...
        plt.xlabel("time"); plt.ylabel("Total Energy Π"); plt.tight_layout()
        fig1.savefig(self.out_dir / "H_vs_time.png", dpi=300); plt.close(fig1)

        fig2 = plt.figure(); plt.plot(self.times, self.Pi_vals)
        plt.xlabel("time"); plt.ylabel("Total Dissipation"); plt.tight_layout()
        fig2.savefig(self.out_dir / "Pi_vs_time.png", dpi=300); plt.close(fig2)

        fig3 = plt.figure()
        plt.plot(self.times, self.dIntU_vals,
         label="Δ∫u", linewidth=2)
        plt.plot(self.times, self.dIntPhi_vals,
         label="0.5 Δ∫φ", linewidth=2,
         linestyle="--",  # dashed
         dash_capstyle="butt")  # clean gaps
        plt.xlabel("time"); plt.ylabel("change in integrals")
        plt.legend(frameon=False)
        plt.tight_layout()
        fig3.savefig(self.out_dir / "balance.png", dpi=300); plt.close(fig3)

        fig4 = plt.figure(); plt.plot(self.times, self.misfit_vals)
        plt.xlabel("time"); plt.ylabel("misfit = Δ∫u - 0.5Δ∫φ")
        plt.tight_layout()
        fig4.savefig(self.out_dir / "misfit.png", dpi=300); plt.close(fig4)
