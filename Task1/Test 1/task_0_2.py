...
# -*- coding: utf-8 -*-
"""
task_1.py — Phase-field (Allen–Cahn-like) with gradient term + diagnostics

Overview
--------
Implicit time stepping for a phase-field order parameter φ on a 2D rectangle,
using P1 Lagrange elements in FEniCSx/dolfinx. Compared to a purely local
bulk model, this script adds the interface energy via a gradient term and
computes several diagnostics useful for T1 verification:

- Area-based interface position x*(t) from the integrated solid indicator
  I = 0.5(φ+1), oriented by `SOLID_ON_LEFT`.
- Interface speed v(t) = dx*/dt.
- Total free energy E(t) = ∫ ( f_bulk(φ) + 0.5 λ₀² |∇φ|² ) dx.
- Centerline thickness δ₀.₈(t) from φ(x, y=Ly/2) crossings of ±0.8.
- Early-exit conditions: edge hit, area saturation, and (MPI-aware) interior
  pointwise saturation.

Weak form (implicit Euler)
--------------------------
Let df/dφ = -φ + φ³ + ζ u (1 - 2φ² + φ⁴). Then the residual is:

    ( τ₀ φ - τ₀ φ₀ + dt * df/dφ, w ) + dt * λ₀² ( ∇φ · ∇w ) = 0

Natural (zero-flux) BCs are implied by the gradient term. No Dirichlet BCs
are imposed.

Outputs
-------
- `phase_grad.xdmf` : time series of φ(t)
- `interface_track.csv` : time, f_solid, interface_pos, interface_speed
- `energy_track.csv`    : time, total_energy
- `thickness_track.csv` : time, δ₀.₈ (centerline thickness)

Notes
-----
- Script is MPI-capable; reductions are used for robust early-exit checks.
- Linear solve uses LU (SuperLU_DIST or MUMPS if available).
...
# ------------------------ calculates fraction of which solid------------------------
...
if msh.comm.rank == 0:
    print(f"Favored phase for u={u:+.3f}: {favored_phase(u, zet)}")  #prints favored phase baseed on u and zet
...
while t < T:
    ...
    # ---------- robust early-exit decisions  ----------   # <<< CHG
